...
def test_calc_var_wei_value():
    ...
    assert calcs.calc_var_value("10gwei - random(2gwei,2gwei)") == to_wei("8gwei")
    ...
    assert calcs.calc_var_value("12.2 gwei") == to_wei("12.2gwei")
    assert calcs.calc_var_value("12.2 eth") == to_wei("12.2eth")
    assert calcs.calc_var_value("12.2 ether") == to_wei("12.2eth")
    assert calcs.calc_var_value("12.2 t", decimals=6) == 12.2 * 10**6

    with pytest.raises(ValueError):
        calcs.calc_var_value("fff")
    with pytest.raises(ValueError):
        calcs.calc_var_value("12.3 gwei + base", var_name="base")
    with pytest.raises(ValueError):
